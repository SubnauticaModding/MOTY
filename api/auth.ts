...
export default async function (data: server.EndpointData) {
  try {
    ...
    if (!member.joinedTimestamp) return data.res.redirect("/?alert=Could not get join date. Let @AlexejheroYTB%231636 know about this.");
    if (member.joinedTimestamp >= 1606780800000) // December 1st 2020, 00:00 UTC
