...
module.exports.enableLoggingProxy = (client) => {
  ...
  client.on('debug', (s) => {
    ...
    client.channels.get(config.consoleChannelID).sendMessage(new Discord.RichEmbed().setAuthor('Debug').setDescription(s));
  });
  client.on('info', (s) => {
    client.channels.get(config.consoleChannelID).sendMessage(new Discord.RichEmbed().setAuthor('Log').setDescription(s).setColor('BLUE'));
  });
  client.on('warn', (s) => {
    client.channels.get(config.consoleChannelID).sendMessage(new Discord.RichEmbed().setAuthor('Warn').setDescription(s).setColor('ORANGE'));
  });
  client.on('error', (s) => {
    client.channels.get(config.consoleChannelID).sendMessage(new Discord.RichEmbed().setAuthor('Error').setDescription(s).setColor('RED'));
  });
};